...
def __compute_local_grandforest_model(expression_data, interaction_network):
	...
	open(config.get_option('TEMP_DIR') + '/' + 'interaction_network.RData', 'wb').write(
	...
	local_model = base64.b64encode(open(config.get_option('TEMP_DIR') + '/' + 'local_model.RData', 'rb').read()).decode(
		'utf-8')

	# save amount of lines - 1 (header line) as weighting constant
	with open(config.get_option('INPUT_DIR') + '/' + config.get_option(
			'expression_data_filename')) as expression_data_file:
		nr_samples = sum(1 for line in expression_data_file) - 1

	print(
		f'[ALGO] Local computation of client {config.get_option("id")}: {sys.getsizeof(local_model)} Bytes with {nr_samples} samples successful')
...
def __aggregate_grandforest_models(global_data):
	...
	open(config.get_option('TEMP_DIR') + '/' + 'forest1.RData', 'wb').write(
	...
	for i in range(1, len(global_data)):
		open(config.get_option('TEMP_DIR') + '/' + 'forest2.RData', 'wb').write(
	...
	global_model = base64.b64encode(open(config.get_option('TEMP_DIR') + '/' + 'forest1.RData', 'rb').read()).decode(
		'utf-8')
...
def local_computation(expression_data, interaction_network):
	local_model, nr_samples = __compute_local_grandforest_model(expression_data, interaction_network)
	return local_model
